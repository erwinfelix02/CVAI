...
// Helper function to format dates into readable text
const formatReadableDate = (dateString?: string) => {
  if (!dateString || dateString === "Recent") return "Recent";
  const parsed = new Date(dateString);
  if (isNaN(parsed.getTime())) return dateString; // Return as-is if already a formatted string
...
export default function StudentCoursesContent() {
  ...
  useEffect(() => {
    async function fetchStudentAndCourses() {
      try {
        console.log("🔍 [DEBUG] Starting student course & material fetch...");
        ...
        // Fetch student record from backend
        if (identifier) {
          try {
            ...
            if (listRes.ok) {
              ...
              if (Array.isArray(listData) && listData.length > 0) {
                ...
                if (matchedStudent.facultyId) {
                  facultyId = matchedStudent.facultyId;
                }
              }
            }
          } catch (apiErr) {
        ...
        console.log("👨‍🏫 [DEBUG] Associated Faculty ID:", facultyId);
        ...
        // Fetch actual materials from backend
        const materialsEndpoint = facultyId ? `/api/materials?facultyId=${encodeURIComponent(facultyId)}` : `/api/materials`;
        console.log(`📡 [DEBUG] Fetching materials endpoint: ${materialsEndpoint}`);
        
        const materialsRes = await fetch(materialsEndpoint);
        const allMaterials: CourseMaterialFile[] = materialsRes.ok ? await materialsRes.json() : [];
        console.log("📦 [DEBUG] Retrieved materials from backend:", allMaterials);

        // Group materials by course code / faculty match and format dates
        ...
        mappedCourses.forEach((course) => {
          const matchedFiles = allMaterials.filter(
            (mat) =>
              ...
              (facultyId && mat.facultyId === facultyId)
          );

          materialsMap[course.id] = matchedFiles.length > 0 
            ? matchedFiles.map(m => ({ ...m, date: formatReadableDate(m.date) })) 
            : [
                {
                  id: `default-${course.id}`,
                  title: `${course.code} - Syllabus & Course Guide.pdf`,
                  sizeLabel: "1.2 MB",
                  date: "Recent",
                ...
                <div>
                  <a
                    ...
                    onClick={(e) => {
                      if (mat.filePath === "#") {
                        e.preventDefault();
                        alert(`Simulation file for ${mat.title}`);
                      }
                    }}
                  >
